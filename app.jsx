    ...
    <div style={S.tabBarWrap}>
      <div style={S.tabBar}>
        ...
        <button style={importOpen ? S.tabImportBtnActive : S.tabImportBtn} onClick={onOpenImport} title="共有リンク／コードから追加">
          <LinkIcon size={14} strokeWidth={2.25} />
          コード
        </button>
      </div>
    </div>
    ...
    <div style={S.pageNav}>
      {items.map((it) => (
        <button key={it.key} style={view === it.key ? S.pageNavBtnActive : S.pageNavBtn} onClick={() => setView(it.key)}>
          <it.icon size={14} strokeWidth={2.25} />
          {it.label}
        </button>
      ))}
    </div>
...
const S = {
  pageNav: { display: "flex", flexWrap: "wrap", gap: 6, rowGap: 8, marginBottom: 16, borderBottom: `1px solid ${T.divider}`, paddingBottom: 10 },
  pageNavBtn: { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 12.5, fontWeight: 600, padding: "8px 12px", borderRadius: 8, border: `1px solid ${T.divider}`, background: T.surface, color: T.muted, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 },
  pageNavBtnActive: { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 12.5, fontWeight: 700, padding: "8px 12px", borderRadius: 8, border: `1px solid ${T.accent}`, background: T.accent, color: "#FFFFFF", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 },
  ...
  scheduleFilterRow: { display: "flex", flexWrap: "wrap", gap: 8 },
  ...
  page: { fontFamily: "-apple-system, BlinkMacSystemFont, 'Hiragino Sans', 'Yu Gothic', sans-serif", color: T.ink, background: T.page, padding: "20px 16px 44px", maxWidth: 560, margin: "0 auto", boxSizing: "border-box", minHeight: "100vh", overflowX: "hidden" },
